...
module.exports = class Contenedor {
    ...
    async save(producto) {
        ...
        try {
            await fs_1.default.promises.writeFile(`./src/${this.archivo}`, JSON.stringify(this.productos));
            return this.maxId;
        }
        catch (error) {
    ...
    async getAll() {
        try {
            const productos = JSON.parse(await fs_1.default.promises.readFile(`./src/${this.archivo}`, 'utf-8'));
    ...
    async deleteAll() {
        this.productos = [];
        try {
            await fs_1.default.promises.writeFile(`./src/${this.archivo}`, JSON.stringify([]));
        }
        catch (error) {
            throw new Error(error);
        }
